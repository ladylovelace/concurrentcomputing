...
int main (int argc, char *argv[]) {
	...
	if( ( file = fopen ( "shakespeare.txt", "r" ) ) != NULL ) {
		...
		if ( myRank == ROOT ) {
			...
			while(  ftell( file ) < processo[myRank - 1].fim ) {
				...
				fscanf( file, "%[^\n]\n" ,sentenca );
				//printf("%s\n", sentenca);
				palindromoSentenca( sentenca, &processo[myRank - 1].qdeSentenca, &processo[myRank - 1].qdePalavra, 
						&processo[myRank - 1].sentencaPrimo ,&processo[myRank - 1].palavraPrimo );
				
			}

			printf( "Slave:%d encontrou %d palavras palindromos e dentre elas %d sao primos\n", myRank, 
				processo[myRank - 1].qdePalavra, processo[myRank - 1].palavraPrimo );
			printf( "Slave:%d encontrou %d sentenca palindromos e dentre elas %d sao primos\n", myRank, 
				processo[myRank - 1].qdeSentenca, processo[myRank - 1].sentencaPrimo );
